const lightColors = {
  mainBg: "bg-gray-50",
  navBg: "bg-linear-to-br from-gray-300 to-white",
  primaryText: "text-gray-900",
  secondaryText: "text-gray-800",
  hoverText: "hover:text-purple-500",
  activeText: "text-purple-600",
  button: "bg-linear-to-br from-purple-500 to pink-500",
  border: "border-gray-300",
  cardBg: "bg-linear-to-br from-gray-950 via-gray-900 to-gray-800",
  cardHover: "hover:to-gray-700",
  cardBorder: "border border-gray-700",
  cardShadow: "shadow-md hover:shadow-xl",
};

const darkColors = {
  mainBg: "bg-gray-900",
  navBg: "bg-linear-to-br from-gray-700 to-black",
  primaryText: "text-white",
  secondaryText: "text-gray-300",
  hoverText: "hover:text-purple-400",
  activeText: "text-purple-400",
  button: "bg-linear-to-br from-purple-500 to pink-500",
  border: "border-gray-700",
  cardBg: "bg-linear-to-tl from-black via-gray-900 to-gray-800",
  cardHover: "hover:from-gray-900 hover:to-gray-700",
  cardBorder: "border border-gray-800",
  cardShadow: "shadow-lg hover:shadow-2xl",
};

export { lightColors, darkColors };
